...
class moodle1_mod_apply_handler extends moodle1_mod_handler {
    ...
    public function process_apply($data) {
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_apply');
        ...
        // start writing apply.xml
        $this->open_xml_writer("activities/apply_{$this->moduleid}/apply.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $this->moduleid,
            'modulename' => 'apply', 'contextid' => $contextid));
        $this->xmlwriter->begin_tag('apply', array('id' => $instanceid));
    ...
    /**
     * This is executed when we reach the closing </MOD> tag of our 'apply' path
     */
    public function on_apply_end() {
        // finish writing apply.xml
        $this->xmlwriter->end_tag('items');
        $this->xmlwriter->end_tag('apply');
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/apply_{$this->moduleid}/inforef.xml");
