...
// $SESSION->apply->current_tab = $do_show;
...
//this page only can be shown on nonanonymous applys in courses
//we should never reach this page
if ($apply->anonymous != APPLY_ANONYMOUS_NO OR $apply->course == SITEID) {
    print_error('error');
}

$url = new moodle_url('/mod/apply/show_nonrespondents.php', array('id'=>$cm->id));
...
if ($action == 'sendmessage' AND has_capability('moodle/course:bulkmessaging', $coursecontext)) {
    ...
    $link2 = $CFG->wwwroot.'/mod/apply/index.php?id='.$course->id;
    $link3 = $CFG->wwwroot.'/mod/apply/view.php?id='.$cm->id;

    $htmlmessage .= '<div class="navbar">'.
    '<a target="_blank" href="'.$link1.'">'.$shortname.'</a> &raquo; '.
    '<a target="_blank" href="'.$link2.'">'.$strapplys.'</a> &raquo; '.
    '<a target="_blank" href="'.$link3.'">'.format_string($apply->name, true).'</a>'.
    '</div>';
...
$PAGE->navbar->add(get_string('show_nonrespondents', 'apply'));
...
$baseurl = new moodle_url('/mod/apply/show_nonrespondents.php');
...
$table = new flexible_table('apply-shownonrespondents-'.$course->id);
...
$matchcount = apply_count_incomplete_users($cm, $usedgroupid);
...
$students = apply_get_incomplete_users($cm, $usedgroupid, $sort, $startpage, $pagecount);
//####### viewreports-start
...
if (!$students) {
    ...
    echo print_string('non_respondents_students', 'apply');
    echo ' ('.$matchcount.')<hr />';

    if (has_capability('moodle/course:bulkmessaging', $coursecontext)) {
        echo '<form class="mform" action="show_nonrespondents.php" method="post" id="apply_sendmessageform">';
    }
    foreach ($students as $student) {
        ...
        if ($DB->record_exists('apply_completedtmp', array('userid'=>$user->id))) {
            $data[] = get_string('started', 'apply');
        } else {
            $data[] = get_string('not_started', 'apply');
        }
    ...
    if ($showall) {
        ...
        echo $OUTPUT->container(html_writer::link($allurl, get_string('showperpage', '', APPLY_DEFAULT_PAGE_COUNT)),
                                    array(), 'showall');
    ...
    if (has_capability('moodle/course:bulkmessaging', $coursecontext)) {
        ...
        echo '<legend class="ftoggler">'.get_string('send_message', 'apply').'</legend>';
        echo '<div>';
        echo '<label for="apply_subject">'.get_string('subject', 'apply').'&nbsp;</label>';
        echo '<input type="text" id="apply_subject" size="50" maxlength="255" name="subject" value="'.$subject.'" />';
        echo '</div>';
        ...
        echo '<input type="submit" name="send_message" value="'.get_string('send', 'apply').'" />';
        echo '</div>';
        ...
        $module = array('name'=>'mod_apply', 'fullpath'=>'/mod/apply/apply.js');
        $PAGE->requires->js_init_call('M.mod_apply.init_sendmessage', null, false, $module);
    }
}
echo $OUTPUT->box_end();
echo $OUTPUT->box_end();
